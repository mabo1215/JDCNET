#!/usr/bin/env bash
set -euo pipefail
# Stage A: BIMCV 510-patient (113+/397-) extended paired CV
# 4 teachers x 4 methods x 5 folds x 3 seeds = 240 runs
# Uses weighted-CE auto class-weights + use_weighted_sampler for the 3.5:1 imbalance

ROOT=${ROOT:-/data/JDCNET_git/src}
PYTHON_BIN=${PYTHON_BIN:-python3}
TAG=${TAG:-bimcv_full_paired_cv_20260516}
CV_DIR=${CV_DIR:-/data1/midrc/bimcv_full_paired_cv_20260516}
...
log "STAGE_A_START tag=$TAG batch=$BATCH_SIZE workers=$NUM_WORKERS concurrency=$CONCURRENCY gpus=[$GPUS]"
cp -n "$DRR_SRC"/bimcv_S*.png "$DRR_SHM"/ 2>/dev/null || true
cd "$ROOT"

# Assumes ops/extract_ct_teacher_variants.py has already populated /dev/shm/bimcv_ct_{mid,3slice,proj}
# (done in a prior step). Re-running is idempotent: it skips already-rendered patients.
...
for gi in "${!gpus[@]}"; do
  ...
  screen -S "stageA_g${gpu}" -X quit >/dev/null 2>&1 || true
  screen -dmS "stageA_g${gpu}" bash "$LOG_DIR/gpu${gpu}_queue.sh"
  log "LAUNCHED gpu=$gpu runs=${#assigned[@]}"
done
log "STAGE_A_ALL_LAUNCHED total=${#names[@]}"
screen -ls | grep stageA || true
